...
export function getContrastColor(bgColor: string): string {
  // HSL colors generated by the app
  ...
  // Hex colors selected from the color input (#RRGGBB)
  const hex = bgColor.trim().match(/^#([0-9a-fA-F]{6})$/);
  if (hex) {
    const value = hex[1];
  ...
  // Basic rgb(...) support if used in future updates
  const rgb = bgColor.trim().match(/^rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)$/);
  if (rgb) {
